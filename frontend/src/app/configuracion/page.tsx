...
export default function ConfiguracionPage() {
  ...
  const sync = async () => {
    ...
    try {
      ...
      setSyncResult(`ok:Sincronizado: ${r.nuevos} carritos nuevos importados`)
    } catch {
      setSyncResult('err:Error al sincronizar. Verifica las credenciales de Shopify.')
    } finally {
      setSyncing(false)
    }
  }
    ...
    <div>
      <div className="mb-10">
        ...
        <p className="text-sm mt-2" style={{ color: 'var(--color-text-muted)' }}>
          Integraciones y configuracion del agente
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        >
          <div className="flex items-center gap-3 mb-5">
            <div className="p-2.5 rounded-xl" style={{ background: '#f5efe8' }}>
          ...
          {loadingStatus ? (
            <div className="flex items-center gap-2 mb-5">
              ...
              <span className="text-sm" style={{ color: 'var(--color-text-muted)' }}>Verificando...</span>
            </div>
          ) : shopifyStatus ? (
            <div className="mb-5">
              <div className="flex items-center gap-2 mb-3">
                {shopifyStatus.valid ? (
                  <><CheckCircle2 size={18} style={{ color: '#5a8a6e' }} />
                  <p className="font-semibold text-sm" style={{ color: '#3d6b4f' }}>Conectado</p></>
                ) : (
                  <><XCircle size={18} style={{ color: '#c2614b' }} />
                  <p className="font-semibold text-sm" style={{ color: '#a34e3b' }}>Sin conectar</p></>
                )}
              </div>
              {shopifyStatus.shop && (
                <p className="text-sm" style={{ color: 'var(--color-text-muted)' }}>
                  Tienda: <strong style={{ color: 'var(--color-text)' }}>{shopifyStatus.shop}</strong>
                </p>
              )}
            </div>
          ) : null}
          ...
          {syncResult && (
            <motion.div
              ...
              style={{
                background: syncResult.startsWith('ok:') ? '#e8f0eb' : '#fef2f0',
                color: syncResult.startsWith('ok:') ? '#3d6b4f' : '#a34e3b',
                borderLeft: `3px solid ${syncResult.startsWith('ok:') ? '#5a8a6e' : '#c2614b'}`,
              }}
            >
              {syncResult.replace(/^(ok:|err:)/, '')}
            </motion.div>
          )}
        </motion.div>
        ...
        >
          ...
          <div className="space-y-2">
            {webhooks.length === 0 ? (
              <p className="text-sm py-4 text-center" style={{ color: 'var(--color-text-muted)' }}>No hay webhooks registrados</p>
            ) : (
              webhooks.map((w) => (
                <div
                  key={w.id}
                ...
                >
                  ...
                  <button
                    ...
                    style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#c2614b', opacity: deletingId === w.id ? 0.5 : 1 }}
                  >
                    <Trash2 size={14} />
                  </button>
                </div>
        ...
        <motion.div
          ...
          style={{ background: 'linear-gradient(135deg, #c2614b 0%, #d4a574 100%)', color: '#faf6f1' }}
        >
          <div className="flex items-center gap-4">
            <div
              className="w-14 h-14 rounded-2xl flex items-center justify-center text-2xl"
              style={{ background: 'rgba(255,255,255,0.2)', fontFamily: 'var(--font-display)', fontWeight: 700 }}
            >
              L
            </div>
            <div>
              ...
              <p className="text-sm mt-0.5" style={{ opacity: 0.7 }}>
                Agente de WhatsApp con IA · Atencion al cliente de vestidos de bano
              </p>
            </div>
            <div className="ml-auto flex items-center gap-2">
              <div className="w-2.5 h-2.5 rounded-full pulse-warm" style={{ background: '#5a8a6e' }} />
              <p className="text-sm font-semibold">Activo</p>
            </div>
          </div>
          <div className="grid grid-cols-3 gap-4 mt-6">
            {[
              { label: 'Proveedor WhatsApp', value: 'Twilio' },
              ...
              <div key={label} className="p-3 rounded-xl" style={{ background: 'rgba(255,255,255,0.15)' }}>
                <p className="text-xs" style={{ opacity: 0.6 }}>{label}</p>
